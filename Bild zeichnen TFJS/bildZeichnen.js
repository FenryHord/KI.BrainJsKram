...
function paintImageFromTrainingData(data, canv) {
  ...
  data.forEach(element => {
    index1 =
      (Math.round(element.input[0] * WIDTH) +
        Math.round(element.input[1] * HEIGHT) * WIDTH) *
      4;
...
async function trainStep() {
  ...
  const tmpX = [];
  const tmpY = [];
  currentTrainData.forEach(element => {
    tmpX.push(element.input);
    tmpY.push(element.output);
  });

  const x = tf.tensor2d(tmpX);
  const y = tf.tensor2d(tmpY);
  x.print();
  y.print();
...
async function paintCurrentNetImage(data) {
  ...
  for (let x = 0; x < WIDTH; x++) {
    for (let y = 0; y < HEIGHT; y++) {
      let rgb = [];
      const inp = tf.tensor2d([[x / WIDTH, y / HEIGHT]]);
      const res = await

      model.predict(inp, { batchSize: 32 });
      if(x == 0 && y == 0) console.log("result", res);

      // net.run([x / WIDTH, y / HEIGHT]);
      let r = rgb[0] * 255;
      let g = rgb[1] * 255;
      let b = rgb[2] * 255;
      ctx.beginPath();
